    ...
    root.querySelectorAll('[data-smartlink]').forEach((el) => {
      ...
      if (el.tagName === 'A') {
        ...
        if (!el.getAttribute('href') || el.getAttribute('href') === '#') {
          el.setAttribute('href', el.dataset.url || smartUrl());
        }
        ...
        el.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          openSmartlink(el.dataset.url || smartUrl(), 'cta-button');
        });
      }
    });
  }
    ...
    root.querySelectorAll('[data-smartlink-card]').forEach((el) => {
      ...
      if (mode === 'toast') {
        el.addEventListener('click', () => {
          ...
          showOfferToast(smartUrl(), 'card-toast');
        });
      } else {
        ensureBadge(el);
        el.setAttribute('role', 'link');
        el.setAttribute('tabindex', '0');
        const open = () => openSmartlink(el.dataset.url || smartUrl(), 'card');
